...
def main():
    ...
    parser.add_argument('-s', '--mpd_stream_hostport', default=os.environ.get('MPD_STREAM_HOSTPORT','127.0.0.1:8000'), help='Hostname/IP and port of Music Player Stream.' )
    parser.add_argument('-y', '--yamaha_host', default=os.environ.get('YAMAHA_HOST','127.0.0.1'),                      help='Hostname/IP of Yamaha AVR Receiver.' )
    ...
    run_daemon(args.mpd_host, args.mpd_stream_hostport, args.yamaha_host, int(args.max_volume_db), args.surround_program)


def run_daemon(mpd_host, mpd_stream_hostport, yamaha_host, max_volume_db, surround_program):
    ...
    while True:
        try:
            ...
            mpd_stream_url = "http://" + mpd_stream_hostport + "/stream.mp3"

            logging.info(f"Connecting to Yamaha AVR Receiver at {rxv_ctrl_url}...")
